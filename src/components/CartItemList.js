...
export const CartItemList = ({ data , updateQuantity , removeCartItem}) => {
  ...
  return (
    <Table>
      ...
      <tbody>
        {data.map((cartItem) => (
          <CartItem key={cartItem.id} product={cartItem.product} quantity={cartItem.quantity} updateQuantity={updateQuantity} removeCartItem={removeCartItem}/>
        ))}

        <tr>
          <td></td>
          <SummaryTableCell>Total</SummaryTableCell>
          <SummaryTableCell>฿{totalPrice}</SummaryTableCell>
        </tr>
      </tbody>
    </Table>
  );
